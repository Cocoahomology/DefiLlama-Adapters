...
import { hourlyPeggedBalances, dailyPeggedBalances } from "../../peggedAssets/utils/getLastRecord";
  ...
  makeCoingeckoCall(url + dateFormatted, 4, getCoingeckoLock).then((res) => {
    if (startTimestamp < timestamp) {
      ...
      if (typeof mcap !== "number") {
        throw new Error(`mcap at timestamp ${timestamp} is NaN. Instead it is ${typeof mcap}.`)
      }
...
const startTimestamp = 1601467216; // I'd rather just fill it in here for now.
const endTimestamp = 1651467216;
